...
int encrypt3DesCfb(char *input, int input_size, char *key, char *iv, char *output)
{
        int outl = 0; 
...
int decrypt3DesCfb(char *input, int input_size, char *key, char *iv, char *output)
{
        int outl = 0;
        int decrypted_total = 0;
        EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
        EVP_DecryptInit(ctx, EVP_des_ede3_cfb(), key, iv);
...
int main(int argc, char *argv[])
{
        ...
        int ciphertext_total = encrypt3DesCfb(message, messageLen, key, iv, ciphertext);
        ...
        decrypt3DesCfb(ciphertext, ciphertext_total, key, iv, decrypted);
